...
export default function Profilepage() {
  ...
  const closePopupAndNavigate = () => {
    ...
    navigate("/welcome"); // Navigate to the welcome page when popup is closed
  };

  useEffect(() => {
    if (!isPopupOpen) {
      document.body.style.overflow = "auto"; // Ensure no body overflow if popup was blocking scroll
    }
  }, [isPopupOpen, navigate]);

  return (
    <div
      className={`background ${isPopupOpen ? "blur" : "hidden"}`}
      onClick={closePopupAndNavigate} // Background click closes the popup
    >
      {isPopupOpen && (
        <div className="pro-container" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="small-column">
            <div className="back-btn">
              <div onClick={closePopupAndNavigate}>
            ...
            <div className="row-2">
              <div className="sub-row2">
                <div className="subrow-left">
                  <FontAwesomeIcon
                    icon={faUserCircle}
                    style={{ fontSize: "19px", padding: "4px", paddingRight: "20px" }}
                    color="rgb(0,0,0,0.5)"
                  />
                  <h5>My Account</h5>
                </div>
                <div>
                  <Link to="/profile">
                    <FontAwesomeIcon
                      icon={faChevronRight}
                      style={{ fontSize: "19px", padding: "4px" }}
                      color="rgb(0,0,0,0.5)"
                    />
                  </Link>
                </div>
